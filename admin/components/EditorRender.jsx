...
function EditorRenderComponent({ content }) {
  if (!content || !content.blocks) return <p>No content to display</p>;
    ...
    <div className="prose space-y-4">
      {content.blocks.map((block, idx) => {
        ...
        switch (block.type) {
          ...
          case "strikethrough": {
            const Tag =
              block.type === "header" ? `h${block.data.level}` :
              block.type === "quote" ? "blockquote" :
              block.type === "marker" ? "mark" :
              block.type === "strikethrough" ? "s" : "p";
          ...
          case "code":
            return (
              <pre key={idx} style={style} className="bg-gray-900 text-white p-3 rounded-md overflow-x-auto">
          ...
          case "embed":
            return (
              <div key={idx} className="my-4">
                ...
                />
                {block.data.caption && (
                  <p className="text-xs text-gray-500 mt-1">{block.data.caption}</p>
                )}
              </div>
            );
