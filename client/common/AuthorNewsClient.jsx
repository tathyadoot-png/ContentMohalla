...
export default function AuthorNewsClient({ initialItems, authorId }) {
  ...
  const fetchMoreData = async () => {
    try {
      const res = await api.get("/news/paginated/list", {
        params: { page: page + 1, limit, author: authorId },
      });
      if (res.data?.length) {
      ...
      console.error("Error loading more author news:", err);
      setHasMore(false);
    }
  };
  ...
  return (
    <div>
      <SectionDivider text="लेखक की अन्य खबरें" />
      <InfiniteScroll
      ...
      >
        <div className="space-y-4">
          {items.map((item) => (
             <Link key={item._id} href={`/news/${getCategorySlug(item)}/${item.slug}`} className="flex ... group">
                {/* ... Your JSX for a single author news item ... */}
